...
public class Banker {
    ...
    public void setMaximumDemand(int customerIndex, int[] maximumDemand) {
        ...
        maximum[customerIndex] = maximumDemand.clone();
    ...
    /**
     * Compares all values of the array, returns false if any element
     * of array1 is greater than array2
     *
     * @param array1 First 1D array
     * @param array2 Second 1D array
     * @return true if all entries in array1 are smaller than array2
     * else false
     */
    ...
    private synchronized boolean checkSafe(int customerIndex, int[] request) {
        ...
        int[][] tempNeed = Arrays.stream(need).map(int[]::clone).toArray(int[][]::new);
        int[][] tempAllocation = Arrays.stream(allocation).map(int[]::clone).toArray(int[][]::new);

        // assume request has been granted, update tables
        for (int i = 0; i < numberOfResources; i++) {
        ...
        // initialise values for work, create new arrays to
        // keep track of finished tasks and if there is deadlock
        ...
        while (isPossible) {
            ...
            for (int i = 0; i < numberOfCustomers; i++) {
                ...
                if (!isFinished[i] && compareArray(tempNeed[i], work)) {
                    isPossible = true;

                    // update values for work
                    for (int j = 0; j < numberOfResources; j++) {
